...
<body>
	...
	if(isset($_POST['submit'])) {
		include __DIR__.'/submit_edit_barang.php';
	}
	?>
	...
	<form action="" method="post" enctype="multipart/form-data">
		...
		$result = mysqli_query($conn, "SELECT * FROM barang NATURAL JOIN kategori_produk where kode_barang='$id'");
		$row = mysqli_fetch_assoc($result);
		?>

		<br>
		kode: <input type="text" value="<?php echo $row['kode_barang']?>" disabled> 
		<input type="hidden" name="kode_barang" value="<?php echo $row['kode_barang']?>">
		<br>
		nama: <input type="text" name="nama_barang" value="<?php echo $row['nama_barang']?>">
		<br>
		harga: <input type="text" name="harga" value="<?php echo $row['harga']?>">
		<br>
		stok: <input type="text" name="stok" value="<?php echo $row['stok']?>">
		<br>
		...
		while ($row_kategori = mysqli_fetch_assoc($result)) {
			if ($row['id_kategori'] == $row_kategori['id_kategori']) {
				echo "<option selected value='" . $row_kategori['id_kategori'] . "'>" . $row_kategori['nama_kategori'] . "</option>";
			}else{
		...
		<br>
		Preview :
		<img id="preview" src="<?php echo $url_website ?>/admin/barang/gambar/<?php echo $row['kode_barang'] ?>.png?nocache=<?php echo time(); ?>" width="200px" alt="Tidak ada gambar" />
		<br>
